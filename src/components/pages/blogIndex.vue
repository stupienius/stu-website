<template>
  <div
    id="blogIndex"
    class="relative min-h-screen overflow-y-hidden bg-[#272829] font-serif text-[#dbd3bf]"
  >
    <div
      id="binary-background"
      class="pointer-events-none fixed left-0 top-[-35px] z-0 h-auto w-screen select-none overflow-hidden text-[#187a13] opacity-10"
    >
      <transition name="appear" module="out-in">
        <pre
          v-if="flag"
          class="select-none whitespace-pre-wrap font-mono text-5xl leading-none"
        >
          {{ binaryString }}
        </pre>
      </transition>
    ...
    <div class="relative z-10 flex w-screen flex-col items-center">
      <header
        class="h-auto w-[95vw] rounded-b-2xl border-4 border-t-[0px] border-solid border-[#b7b8b8] bg-[#9b9b9b] py-5 font-serif text-[#61677A]"
      >
        ...
        <div id="blogList">
          <ul class="flex w-screen flex-col items-center justify-center">
            <li
              ...
              :key="post.slug"
              class="m-3 flex w-[60%] list-none rounded-lg bg-[#474b59] p-3 px-5 transition-all duration-500 ease-in hover:bg-yellow-500 hover:text-orange-950"
            >
              <RouterLink
                :to="`/blog/${post.slug}`"
                class="flex size-full flex-col gap-3"
              >
